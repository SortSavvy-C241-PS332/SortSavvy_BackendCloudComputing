...
const getTotalScan = async (request, h) => {
    ...
    const getScanQty = async (id) => {
        const [rows] = await sequelize.query('SELECT COUNT(*) as count FROM total_scan WHERE id = ?', {
            replacements: [id],
            type: sequelize.QueryTypes.SELECT
        });
        return rows[0] && rows[0].count ? rows[0].count : 0; // Tambahkan pengecekan untuk undefined
    };

    try {
        const scanQty = await getScanQty(id);
        return h.response({ id, scanQty }).code(200);
    } catch (error) {
        console.error(error);
